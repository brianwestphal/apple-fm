...
/** The built-in tools, keyed by name. All run locally — apple-fm makes no network calls. */
...
export function toolGuidancePrompt(registry: ToolRegistry): string {
  ...
  const intro =
    ...
    'refusing: never claim you cannot read files or run commands — call the matching tool ' +
    'instead. Match the task to the tool: a local file path → read; a shell command → bash. ' +
    'Call ONE tool for the job, not several for the same thing. Do not use bash to read ' +
    'files, and never invent a tool or write a tool call as plain text. Available tools:';

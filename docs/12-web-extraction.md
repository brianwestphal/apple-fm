...
> **Status:** Shipped (AFM-39). Dependency-free, in `src/tools/builtin/web.ts`
> (`htmlToText`, `windowText`). Unit-tested in `tests/web.test.ts`.
...
- **WX-8 Line-range paging.** `offset` is character-based; a line/paragraph-based page
  unit may read more naturally for the model. (Tracked as AFM-40.)
...
`APPLE_FM_WEB_MAX_CHARS` env var (WX-7).

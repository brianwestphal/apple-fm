...
- **FR-14 Tool calling (extensible, permission-gated)** — Shipped. **Phases 1–4 + on-device verified**: generic round-trip plumbing (Swift `DynamicTool` suspends each call, round-tripped to Node over a `tool_call`/`tool_result` extension of `--session`, bound at `reset`), an extensible `ToolRegistry` (`src/tools/`), the `read` / `bash` / `web` built-ins, and a per-call `PermissionPolicy` gate (`ask`/`allow`/`deny`, REPL `[y/N/a]`, `--allow-tool`/`--deny-tool`/`--yes`/`/tools`, deny-by-default non-interactive; a denial is fed back as a tool result so the model continues). `web` is off by default and reshaped NFR-1 (gated network); its HTML→text uses readability-style main-content extraction + paging (`offset`) + a configurable cap (`APPLE_FM_WEB_MAX_CHARS`) so the tiny window gets mostly real content (AFM-39, `docs/12-web-extraction.md`). Remaining follow-up: a `web` **search** backend (TC-9). Reqs `docs/9-tool-calling.md` + `docs/10-permissions.md` + `docs/11-builtin-tools.md` + `docs/12-web-extraction.md`; design `docs/8-tool-support.md`.
